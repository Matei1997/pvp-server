...
class Vehicle {
     ...
     */
    public setTuningMod(modIndex: number, value: number): void {
        if (!this._vehicle || !mp.vehicles.exists(this._vehicle)) return;
        if (modIndex < 0 || modIndex >= 100) return;
        if (!this._mods.tunningMods) this._mods.tunningMods = {};
        this._mods.tunningMods[modIndex] = value;
        if (modIndex === 18) this._vehicle.setVariable("boost", value === 1 ? 1.3 : 0);
        if (modIndex === RageShared.Vehicles.Enums.VEHICLEMODS.WINDOW_TINT) {
     ...
     */
    public getTuningMods(): { [key: number]: number } {
