class _Vehicle {
    constructor() {
        ...
        mp.events.add("client::vehicle:applyTuningMod", this.applyTuningMod.bind(this));
    }
    ...
    /**
     * Applies a single tuning mod (server-authoritative sync to clients in range).
     */
    public applyTuningMod(vehicleId: number, modIndex: number, value: number) {
        const vehicle = mp.vehicles.atRemoteId(vehicleId);
        if (!this.doesExist(vehicle)) return;
        if (modIndex === 55) {
            vehicle.setWindowTint(value);
        } else {
            vehicle.setMod(modIndex, value);
        }
    }
}
